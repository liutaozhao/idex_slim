...
void MarlinUI::draw_status_screen() {
  ...
  tft.add_bar(0, 0, TFT_WIDTH - 1, 82, COLOR_BACKGROUND);
  tft.add_rectangle(0, 0, TFT_WIDTH - 1, 82, COLOR_COLD);
  if (progress)
  ...
  y = 290;
  // status message
  tft.canvas(0, y, TFT_WIDTH -60 , 27);
  tft.set_background(COLOR_BACKGROUND);
  tft_string.set(status_message);
  tft_string.trim();
...
void MarlinUI::move_axis_screen() {
  ...
  // aligned with x+
  TERN_(HAS_TFT_XPT2046, add_control(xplus_x, TFT_HEIGHT - Y_MARGIN - BTN_HEIGHT - 60 , BUTTON, (intptr_t)disable_steppers, imgBtn39Rounded, !busy));
  TERN_(HAS_TFT_XPT2046, add_control(xplus_x, TFT_HEIGHT - Y_MARGIN - BTN_HEIGHT+10, BUTTON, (intptr_t)level_bed, imgLeveling, !busy));
